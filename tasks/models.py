...
class MaintenanceTask(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=TaskStatus.choices, default=TaskStatus.PLANNED
    )
    ...
    def clean(self):
        ...
        if self.status == self.TaskStatus.IN_PROGRESS:
            for task_tool in self.tools_used.select_related("tool"):
                tool = task_tool.tool
                conflict_exists = (
                    tool.task_tools.exclude(task=self)
...
class GardenBedTask(models.Model):
    bed = models.ForeignKey(
        GardenBed, on_delete=models.CASCADE, related_name="bed_tasks"
    )
    task = models.ForeignKey(
        MaintenanceTask, on_delete=models.CASCADE, related_name="bed_tasks"
    )
...
class MaterialUsage(models.Model):
    ...
    quantity_used = models.DecimalField(
        max_digits=7, decimal_places=2, validators=[MinValueValidator(0.01)]
    )
